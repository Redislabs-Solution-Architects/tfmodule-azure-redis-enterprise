...
# # I think we need a separate backend address pool for each node, with a single IP in it


# resource "azurerm_lb_backend_address_pool" "backend_pool" {
#   resource_group_name = "${azurerm_resource_group.resource.name}"
#   loadbalancer_id     = "${azurerm_lb.redislb.id}"
#   name                = "RedisBackendPool-${count.index}"
#   count = var.node-count
# }

# resource "azurerm_lb_rule" "basic-redis-nats" {
#   count                          = var.node-count
#   resource_group_name            = "${azurerm_resource_group.resource.name}"
#   loadbalancer_id                = "${azurerm_lb.redislb.id}"
#   name                           = "basic-redis-nats-${count.index}"
#   protocol                       = "tcp"
#   frontend_port                  = 10002
#   backend_port                   = 10002
